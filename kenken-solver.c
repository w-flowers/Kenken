...
int solve_kenken(struct kenken *kkptr){
	...
	for(struct node_ctr *dmy = local_ctrs_head; dmy!=NULL; dmy = dmy->next_node){
		...
		//if(testval) printf("P rca");
	}
	
	//int testval2 = reduce_by_grid(psqrs, sqrnumnd);
	//if(testval2) printf("P rbg");
	/*
	for(struct node_ctr *dmy = local_ctrs_head; dmy!=NULL; dmy = dmy->next_node){
		reduce_constraint_arrays_2(dmy->constraint, psqrs);
	}*/
	...
	for(struct node_square *dmy2 = local_ctrs_head->constraint.numbers; dmy2 != NULL; dmy2 = dmy2->next_node){
		if(dmy2->next_node != NULL){
			struct node_square *nxt_sqr = malloc(sizeof(struct node_square));
			nxt_sqr->pos[0] = dmy2->next_node->pos[0];
			nxt_sqr->pos[1] = dmy2->next_node->pos[1];
	...
	for(struct node_ctr *dmy_ptr = local_ctrs_head; dmy_ptr != NULL;){
		...
		//printf("finished dealllocing suqares\n%d\n", (int) dmy_ptr);
...
int reduce_constraint_arrays_2(struct constraint *ctr, struct pzlsqr psqrs[6][6]){
	...
	int res = ctr->result;
	//int x = ctr.numbers->pos[0];
	//int y = ctr.numbers->pos[1];
	if(ctr->op == NOOP){
	...
	//need to implement solution for more than 2 elements
	if(ctr.op == MULTOP){
		for(int i = 0; i < 6; i++){
			int n = psqrs[x][y].avlbl_vals[i];
			if(n != 0){
	...
	if(dmynxt != NULL){
		if(psqrs[dmynxt->pos[0]][dmynxt->pos[1]].solved != -1){
			...
			for(int i = 0; i < 6 && psqrs[dmynxt->pos[0]][dmynxt->pos[1]].avlbl_vals[i]; i++){
				...
				else if(recursive_helper_f_mult(dmynxt->next_node, dmyprv, psqrs, test_result, result)) return 1;
			}
		}
	}
	else if(dmyprv != NULL){
		if(psqrs[dmyprv->pos[0]][dmyprv->pos[1]].solved != -1){
			...
			for(int i = 0; i < 6 && psqrs[dmyprv->pos[0]][dmyprv->pos[1]].avlbl_vals[i]; i++){
				...
				else if(recursive_helper_f_mult(NULL, dmyprv->prev_node, psqrs, test_result, result)) return 1;
			}
		}
	}
	return 0;
	...
	if(dmynxt != NULL){
		if(psqrs[dmynxt->pos[0]][dmynxt->pos[1]].solved != -1){
			...
			if(dmynxt->next_node == NULL && dmyprv == NULL){
				...
				if(recursive_helper_f_add(dmynxt->next_node, dmyprv, psqrs, test_result)) return 1;
			}			
		}
		else{
			for(int i = 0; i < 6 && psqrs[dmynxt->pos[0]][dmynxt->pos[1]].avlbl_vals[i]; i++){
				...
				else if(recursive_helper_f_add(dmynxt->next_node, dmyprv, psqrs, test_result)) return 1;
			}
		}
	}
	else if(dmyprv != NULL){
		if(psqrs[dmyprv->pos[0]][dmyprv->pos[1]].solved != -1){
			...
			if(dmyprv->prev_node == NULL){
				...
				if(recursive_helper_f_add(NULL, dmyprv->prev_node, psqrs, test_result)) return 1;
			}		
		}
		else{
			for(int i = 0; i < 6 && psqrs[dmyprv->pos[0]][dmyprv->pos[1]].avlbl_vals[i]; i++){
				...
				else if(recursive_helper_f_add(NULL, dmyprv->prev_node, psqrs, test_result)) return 1;
			}
		}
	}
	return 0;
...
int brute_fill_grid_2(struct node_ctr *curr_cst, struct node_square *curr_sqr, struct pzlsqr psqrs[6][6], struct pzlsqr psqrs2[6][6], int init_sol[6][6], struct node_ctr *init_csts){
	...
	if(curr_sqr->next_node == NULL){
		if(curr_cst->next_node == NULL){
			...
			if(psqrs[hori_ind][vert_ind].solved != -1){
				...
				return bfg2_has_mult_sol(curr_cst, curr_sqr, psqrs, psqrs2, dmypsqrs, init_sol, init_csts);
			}
			else{
		...
		//below walks to the first node of the next constraint
			...
			if(psqrs[hori_ind][vert_ind].solved != -1){
				...
				if(!valid_constraint(&(curr_cst->constraint))) return 0;
				return brute_fill_grid_2(curr_cst->next_node, curr_cst->next_node->constraint.numbers, psqrs, psqrs2, init_sol, init_csts);
			}
			else{
				...
				while(psqrs[hori_ind][vert_ind].avlbl_vals[0]){
					...
					if(!valid_constraint(&(curr_cst->constraint))){
						...
						int nxtsqrres = brute_fill_grid_2(curr_cst->next_node, curr_cst->next_node->constraint.numbers, psqrs, psqrs2, init_sol, init_csts);
	...
	//below walks to the next node of the current constraint
		...
		if(psqrs[hori_ind][vert_ind].solved != -1){
			...
			return brute_fill_grid_2(curr_cst, curr_sqr->next_node, psqrs, psqrs2, init_sol, init_csts);
		}
		else{
			...
			while(psqrs[hori_ind][vert_ind].avlbl_vals[0]){
				psqrs[hori_ind][vert_ind].solved = psqrs[hori_ind][vert_ind].avlbl_vals[0];
				curr_sqr->entry = psqrs[hori_ind][vert_ind].solved;
				int nxtsqrres = brute_fill_grid_2(curr_cst, curr_sqr->next_node, psqrs, psqrs2, init_sol, init_csts);
				if(nxtsqrres) return nxtsqrres;
				else{
					remove_available(psqrs[hori_ind][vert_ind].avlbl_vals, psqrs[hori_ind][vert_ind].solved);
				}
...
int bfg2_has_mult_sol(struct node_ctr *curr_cst, struct node_square *curr_sqr, struct pzlsqr psqrs[6][6], struct pzlsqr psqrs2[6][6], struct pzlsqr dmypsqrs[6][6], int init_sol[6][6], struct node_ctr *init_csts){
	if(init_sol[0][0]){
		...
		int mult_sol = brute_fill_grid_2(init_csts, init_csts->constraint.numbers, psqrs2, psqrs3, arr, init_csts);
...
int reduce_by_grid_2(struct pzlsqr psqrs[6][6], int hori_ind, int vert_ind){
	...
	for(int k = 0; k < 6; k++){
		if(psqrs[k][vert_ind].solved != -1) remove_available(psqrs[hori_ind][vert_ind].avlbl_vals, psqrs[k][vert_ind].solved);
	}
	for(int k = 0; k < 6; k++){
		if(psqrs[hori_ind][k].solved != -1) remove_available(psqrs[hori_ind][vert_ind].avlbl_vals, psqrs[hori_ind][k].solved);
	}
	return 0;
}
...
int copy_pzlsqrs(struct pzlsqr psqrs2[6][6], struct pzlsqr psqrs[6][6]){
	for(int i = 0; i < 36; i++){
		...
		for(int j = 0; j < 6; j++) psqrs2[i%6][i/6].avlbl_vals[j] = psqrs[i%6][i/6].avlbl_vals[j];
	}
	return 0;
}
