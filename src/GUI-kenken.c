...
{
   if(snprintf(TYPEFACE, sizeof(TYPEFACE), "%s/%s", FILEPATH, "OpenSans-Regular.ttf") >= sizeof(TYPEFACE))
   {
   ...
   else
   {
      ...
      int menu_ret = run_menu(&args);

      if( menu_ret == QUIT_MENU )
      {
         goto EXIT;
      }
      ...
      else
      {
         ...
         while(!quit)
         {
            ...
            if( e.type == SDL_MOUSEBUTTONUP ){
               ...
               //Save kenken - behaviour to be implemented
               if( e.button.x > savekk.btn.x && e.button.x < savekk.btn.x + savekk.btn.w &&
                     e.button.y > savekk.btn.y && e.button.y < savekk.btn.y + savekk.btn.h ){
